...
ggplot(weather, aes(y = temp3pm, x = temp9am, color = location)) + 
  geom_point() + 
  labs(x = "Temperature at 9 AM (Degrees Celsius)", y = "Temperature at 3 PM (Degrees Celsius)", title = "Daily Temperature Shifts in Australia", color = "Location")  
```
...
```{r}
#| fig-cap: "FIGURE 1. 9 AM versus 3 PM temperatures in a given day in the Australian cities of Hobart, Uluru, and Wollongong"

ggplot(weather, aes(y = temp3pm, x = temp9am, color = location)) + 
  geom_point() + 
  labs(x = "Temperature at 9 AM (Degrees Celsius)", y = "Temperature at 3 PM (Degrees Celsius)", color = "Location")  
```
...
#| echo: false
#| fig-cap: "Density plots of 3pm temperatures in 3 Australian locations."
#| fig-alt: "A density plot of 3pm temperatures in Hobart, Uluru, and Wollongong, Australia. Uluru has the highest temperatures, but also displays a wider ranger than Hobart or Wollongong. Data from Macalester Statistics Department"
...
For those with Protanopia, our viz is somewhat hard to read, for those with Tritanomaly, it is very hard to read, and for those with Tritanopia and Monochromacy, it is near-illegible. We cannot rely and singular value color distinctions: it may be very useful to incorporate brightness into our visualizations
...
Because the excessively wide scale not only minimizes the impact of climate change, but also makes any data nearly illegible
...
It depends how you look at it: if we are talking shown visualizations themselves, they emphasize visibility (many different categorical variables, and most common data is acknowledging people of origin/where you come from) and Emotion & Embodiment (They often record highly personal information about the experiences and history of either the creator or displayer of the visualization). On the other hand, the article on whole and the concept of "Decolonizing Data" is more in line with Power (Challenging the eurocentric narrative developed around data and advocating for historical and contemporary indigenous viz) and Pluralism (Acknowledging several different indigenous perspectives and comparing them with dominant styles of viz)
-   improve upon what we might be able to convey with a simpler bar chart, scatterplot, etc?
It shows a lot of different ways to represent variables, and also encourages us to draw on different traditional methodologies of encoding knowledge and data in how we construct our viz
...
I looked at the 2013 budget viz
-   What story does the data graphic tell? What is the main message that you take away from it?
While there were moderate increases in sections like Medicare and Medicaid, and SSI, these were mainly mandatory, and the biggest changes in discrentionary spending were  cuts to military spending on Procurement, Operation and Maintainence, R&D, and Personnel, to meet the budget cap.
