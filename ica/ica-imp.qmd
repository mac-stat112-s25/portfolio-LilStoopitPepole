...
**Data was compiled by Kaggle, a data science learning site, from a movie database called TMBd, using their included API**
...
```{r}
library(tidyverse)
imdb_messy <- read_csv("C:\\Users\\merce\\Documents\\MAC sophmore year\\data science\\portfolio\\portfolio-LilStoopitPepole\\data\\imdb_5000_messy.csv")
```
...
The minimum, maximum, median, and 1st and third quartiles
-   What type of info is provided on categorical variables?
How many times a given value shows up
-   What stands out to you in these summaries? Is there anything you'd need to clean before using this data?
There seems to be a lot of repeats on categorical variables, certainly on color (which is inconsistent) but also possibly on things like movie_title, although they may be movies with the same title rather than duplicate entries. In quantitative variables, there are a lot of NAs, and a good few in the categorical variables too, which is fine, we just would have to figure out what they mean and if we need to remove them are rename them depending on the column and what data we want to analyze
...
There are 3 names for Color movies, 2 for Black and White movies, and it is unclear what NA means
...
There are roughly 4800 movies that are in color (with the value either being "Color","COLOR",or "color"), about 210 movies in black and white (with a value of either "B&W" or "Black and White"), and 19 movies for which color is Not Applicable, which is immediately apparent as to meaning
...
It's much faster, and it is clearly reproducible from the starting point of the publically available data
...
imdb_temp <- imdb_messy |>
  mutate(color=
    fct_recode(color, 
               Color="COLOR", Color="color", Black_White="Black and White", Black_White="B&W"))
  summary(imdb_temp)
...
str_replace(color, 
  ...
  summary(imdb_temp)
  count(imdb_temp, color)
  #although, it changes it from a factor to character string, which makes the summary less valuable. we can always change it back, but this is generally clunkier, unless im missing something
```
...
Associated values will be missing when something is absent, eg, when actor_3_name is NA, so is actor_3_facebook_likes. This might mean that in these cases, there literally aren't 3rd actors in these films. Its not immediately clear which films are more likely to have NAs, but one example near the top is Doug Walker's review of Star Wars the Force Awakens, which is officially a documentary, but is really a youtube video, and lacks lots of variables, both answerable (color, language) and less-so (gross, budget)
...
All 7 of these documentaries have no actors listed, presumably consisting mostly of archival footage, and therefore have no actor facebooks to pull likes from. 
```{r}
imdb_messy |>
  filter(is.na(actor_1_facebook_likes)==TRUE)
```
...
It removed every single row with an NA in any coloumn, leaving only 6 rows
-   In what situations might this function be a good idea?
Where you are comprehensively analyzing all variables,therefore any NA renders a datapoint useless
-   In what situations might this function be a bad idea?
when there are several variables where NA is expected or meaningful, or where you only need to see certain variables.
...
imdb_color <- imdb_messy |>
  filter(!is.na(color) & !is.na(duration))
imdb_color
nrow(imdb_color) 
nrow(imdb_messy)
#we only lost like 30 rows or smth, rather than nearly all of them. this gives a much better dataset but still excludes NAs in the variables that matter
...
imdb_temp |>
  arrange(actor_1_facebook_likes)
```
...
```{r}
animes <- read_csv("../data/anime/AnimeList.csv")
spec(animes)
animes |>
mutate(across(where(is.character), as.factor)) |>
summary()
#did not know this included some less than savory offerings, although as long as we don't target them, shouldn't make for too much trouble. otherwise, all in all, this is a super clean dataset, all things considered. there might be some dupes with cyberborg 009 and a.i. am human, but its hard to tell without an investigation that goes beyond preliminary
```
