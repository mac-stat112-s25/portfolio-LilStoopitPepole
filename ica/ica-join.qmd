...
The student in the class. Also, data on ANTH101

-   Relative to this key, what info does `enrollments_1` have that `students_1` doesn't?
the number of enrollments in the class. Also, data on ART101
...
students_1 sets the unit of observation, student, and enrollments_1 adds the value of enrollments

-   What, if anything, would change if we reversed the order of the data tables? Think about it, then try.
Class would be the furthest left, and student would be NA for ART101
...
-   What did this do? What are the roles of `students_1` (the *left* table) and `enrollments_1` (the *right* table)?
It only joined data for which both dataframes shared a key value, and therefore preserves both units of observation

-   What, if anything, would change if we reversed the order of the data tables? Think about it, then try.
Probably, the position of class and student would just be switched
Oh jeez, student got sent all the way over to the right!
...
-   What did this do? What are the roles of `students_1` (the *left* table) and `enrollments_1` (the *right* table)?
class, the key variable, became the unit of observation, and all rows that had any defined values were included

-   What, if anything, would change if we reversed the order of the data tables? Think about it, then try.
again, student would be shunted all the way to the right, i reckon

The order also changed! that makes sense. It conformed to the first table, and added the secondary to the end
...
It tells us if the observations on the left are represented in the dataframe on the right (ie, they are in the THD building)

-   How does `semi_join()` differ from `inner_join()`?
It does not actually add data from the other dataframe, just filters the coloumns of the left dataframe

-   What, if anything, would change if we reversed the order of the data tables? Think about it, then try.
Likely, would include enrollment instead of students, but would include same classes (observations)
...
-   What did this do? What info would it give us?
It only included observations not present in the right dataframe, ie, not in the THD building

-   What, if anything, would change if we reversed the order of the data tables? Think about it, then try.
It would just show ART101 instead of ANTH101, and would again show enrollements instead of student
(like if you agree. I liked!)
...
The key variables are different names! course and class
...
There are no matches between the left and right dataframe for both key variables (class and grade)
```{r}
students_3 |> 
...
grade.x is the value of grade from the left dataframe, and grade.y is the value of grade from the right dataframe
...
I think I just need course_combined...
...
-   Which of the 2 datasets do you need to answer this question? One? Both?
I will definitely need both
-   If you need course information, use `courses_combined` not `courses`.
-   Do you have to do any joining? If so, which dataset will go on the left, i.e. which dataset includes your primary observations of interest? Which join function will you need?
grades ought to go on the left, since its per student!

```{r}
student_class_size <- grades |>
  left_join(courses_combined) |>
    group_by(sid) |>
    summarize(med_size = median(enroll))
student_class_size
```
...
The most common median class size by a good deal is right around 20, but generally the range between 18 and 24 is fairly typical. Although in fewer numbers, that is small chunk of students with media class sizes between 25 and 30, descending as we approach 30. Median class sizes past 30 are rare, with only about 1-3 students for each value, the maximum being 1 student with a median class size of 38. On the lower side, there a good few students who have median class sizes between 16 and 17, but frequency drops off steeply below that, with only 2 students at the minimum, having median class sizes of 12.

```{r}
 ggplot(student_class_size, aes(x = med_size)) + 
   geom_histogram(color="black",fill="coral2", binwidth=1)
```
...
grades |>
  ...
  summarize(n=n()) |>
  mutate(frac=n/sum(n))
#can you count S and AU as "below B+"? It's hard to tell, so I left them seperate
```
...
grades |>
  ...
  left_join(
    courses |>
      filter(
        (add_count(courses,sessionID))$n==1
      )
    ) |>
  group_by(dept) |>
  summarize(gpa= mean(gp, na.rm=TRUE)) |>
