...
str(weather)
#the units of observation are individual days, also locations
# How many data points do we have? 
nrow(weather)
#2367 data points!
# What type of variables do we have?
#we have date, character, number, and integer variables
```
...
ggplot(weather, aes(y = temp3pm, x = temp9am, color=location)) + 
  geom_point(alpha=0.7, aes(shape=raintoday))
```
...
# Play around!
ggplot(weather, aes(y = temp3pm, x = temp9am, color=location)) + 
  geom_text(alpha=0.7,size=3.5,aes(label=raintoday))
```
...
ggplot(weather, aes(x = raintoday, fill=raintomorrow)) + 
  geom_bar(position = "fill") +
  facet_wrap(~ location) 
#i tried like a million ways. it would probably work better if i changed something like position but i wanted to not disrupt structure. color and alpha are both illegible. honestly, something other than a bar graph might work better. good old facet. nothing beats facet
```
...
ggplot(education, aes(x=sat)) +
  geom_density(color="white",fill="skyblue")
```
...
There are lots of states with average scores in the 900 range, and also a good number in a broader 1030 range, but there are a lot  fewer States with average scores in the 960-ish range
...
There seems to be pretty vague/weak, but somewhat *negative* relationship between spending, both on salary and per student, and SAT scores
...
```{r}
ggplot(education, aes(y=sat,color=expend,x=salary)) +
  geom_point(size=1.5) 
```
...
```{r eval = FALSE}
ggplot(education, aes(y = sat, x = salary, color = cut(expend, 5))) + 
  geom_point() + 
  geom_smooth(se = FALSE, method = "lm")
```

Describe the trivariate relationship between `sat`, `salary`, and `expend`.
SAT scores mostly decrease with spending on both salary and per student, but the negative relationship between SAT scores and teacher salary is weaker in states where more is spent per student, becoming actually positive past 8550 dollars spent per student
...
No. Within all groups, SAT scores increase somewhat as spending increases, even if there are large differences in scores between groups of states which administer more or fewer SATs to students

```{r}
ggplot(education, aes(y=sat,x=expend,shape=fracCat)) +
  geom_point(size=1.5) +
  geom_smooth(method="lm")
```
...
Because education spending was linked with a second predictor variable, the number of SATs administered, which has a negative relationship with SAT scores, as SATs are only administered to students who have the money, time, and desire to prepare for them
