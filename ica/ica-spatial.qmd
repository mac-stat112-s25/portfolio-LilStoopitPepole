...
The plot shows that at a, energy use can range anywhere from ~460 to 25 therms, but is usually either in the vicinity of ~380 or of 55 therms, while at b, energy use tops out at just shy of 400 therms, and can approach nearly 0, but is usually around 45 therms.

```{r}
ggplot(energy, aes(y=therms,x=address)) +
...
At both addresses, energy use peaked during winters (surrounding january), but did not seem to signficantly change between full years
```{r}
...
typically, energy use was higher at both a and b address after renovation, with their low energy use peaks shrunk and their energy use peaks grown, although in b they were shifted somewhat left (that is, towards less therm use)
```{r}
...
-   After renovation...
-   HDD increased, that is, outside temperature decreased...
-   which then leads to higher energy use.

Thus, *when controlling for* HDD (outside temperature),
...
Let's load [the data](https://hash-mac.github.io/stat112site-s25/data/our_fave_places.csv) and map it!
...
Its very difficult to see what countries they're in, except when its obvious because of density, like the USA
...
# we have to reference the dataset in the layer
ggplot(world_boundaries) + 
  geom_sf() + 
  geom_point(
    data = starbucks,
...
Starbucks are most concentrated in The United States, England, Japan, Netherlands, South Korea, and the northeast coast of china 
...
#it did these kooky area guys
# What changed in our code?!
#we changed it points to density_2d
ggplot(cma_boundaries) + 
  geom_sf() + 
  geom_density_2d(
    data = starbucks_cma,
...
ggplot(elections_by_counties, aes(map_id = county_fips, fill = median_rent)) +
  geom_map(map = county_map) +
  scale_fill_distiller(palette="YlGnBu", name = "Median rent") +
  expand_limits(x = county_map$long, y = county_map$lat) +
  theme_map() +
...
ggplot(elections_by_counties, aes(map_id = county_fips, fill = median_age)) +
  geom_map(map = county_map) +
  scale_fill_distiller(palette="YlOrRd", name = "Median Age") +
  expand_limits(x = county_map$long, y = county_map$lat) +
  theme_map() +
